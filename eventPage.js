...
function draw(str,tab){
	...
	browser.tabs.executeScript(null,{code: "show_html(\'"+str+"\',true)"});
}
function chkExecOK(successFunc,errorFunc){
	var executing = browser.tabs.executeScript(null,{code: ""});
...
browser.pageAction.onClicked.addListener(function(tab){
	chkExecOK(
		...
		function(){
			browser.pageAction.setPopup({tabId: tab.id,popup: "showQRcode.html"})
		}
	);
});